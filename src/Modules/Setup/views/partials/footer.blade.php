<footer class="p-6 bg-white shadow-sm dark:bg-gray-800 mt-auto">
    <div class="flex flex-row flex-wrap">
      <div class="flex-shrink w-full max-w-full text-center lg:w-1/2 lg:text-start">
        <ul class="space-x-3 ps-0">
          <li class="inline-block">
            <a class="hover:text-indigo-500" href="/#">Support</a>
          </li>
          <li class="inline-block">
            <a class="hover:text-indigo-500" href="/#">Help Center</a>
          </li>
          <li class="inline-block">
            <a class="hover:text-indigo-500" href="/#">Privacy</a>
          </li>
          <li class="inline-block">
            <a class="hover:text-indigo-500" href="/#">Terms of Service</a>
          </li>
        </ul>
      </div>
      <div class="flex-shrink w-full max-w-full text-center lg:w-1/2 lg:text-end">
        <p class="mt-3 mb-0 md:mt-0">
          <a href="/landing.html" class="hover:text-indigo-500">Taildash</a> | All right reserved
        </p>
      </div>
    </div>
  </footer>
