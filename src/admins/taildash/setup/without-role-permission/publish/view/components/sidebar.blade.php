...
<aside id="sidebar-menu" x-description="Mobile menu" x-bind:aria-expanded="sidebar"  class="fixed {{$theme_setting->sidebar_color}} w-64 h-screen transition-all duration-500 ease-in-out bg-white shadow-sm -ms-64 md:ms-0">
    <div class="h-full overflow-y-auto scrollbars">
      ...
      <ul id="side-menu" x-data="{ selected : 1 }" class="w-full float-none flex flex-col font-medium ps-1.5">
        <li class="relative">
            <a class="active block w-full py-2 px-6 clear-both whitespace-nowrap [&.active]:text-indigo-500 hover:text-indigo-500" href="{{route('admin.dashboard')}}">
        ...
        @foreach ($menus as $menu)
            <x-sidebar-list :menut="$menu" :ml="$loop->iteration"/>
        @endforeach
